...
int main(int argc, const char *argv[]) {
    ...
    key_t key2 = ftok("magazyn2_konf.txt", 75);
    int shmid2 = shmget(key2, 256, 0666 | IPC_CREAT);
    char *data2 = shmat(shmid2, (void *) 0, 0);
    strncpy(data2, buf, 256);
    ...
    if (fork() == 0) { //POTOMNY
        while (1) {
            ...
            sscanf(data2, "%d %d %d %d", &magazyn_A, &magazyn_B, &magazyn_C,
            ...
            if (liczba_A <= magazyn_A && liczba_B <= magazyn_B && liczba_C <= magazyn_C) {
                ...
                printf("Kurier 1, Odebrał zamówienie nr %d: %d %d %d\n", id_zamowienia, liczba_A, liczba_B, liczba_C);
                printf("Magazyn 2, Nie ma wystarczającej ilości towaru.\n");
                podnies_sem(semid, 0); //semafor
                exit(0);
            }

            sprintf(buf, "%d %d %d %d", magazyn_A, magazyn_B, magazyn_C, gold); //zapisz ilości do bufora
            strncpy(data2, buf, 256); //zapisz ilości do pamięci współdzielonej
            podnies_sem(semid, 0); //semafor
        ...
        if (fork() == 0) { // POTOMNY
            ...
            if (fork() == 0) { // POTOMNY
                ...
                printf("Stan magazynu 2 : A:%d B:%d C:%d\n", magazyn_A, magazyn_B, magazyn_C);

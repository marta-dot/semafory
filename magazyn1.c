...
int main(int argc, const char *argv[]) {
    ...
    key_t key1 = ftok("magazyn1_konf.txt", 65);
    int shmid1 = shmget(key1, 256, 0666 | IPC_CREAT);
    char *data1 = shmat(shmid1, (void *) 0, 0);
    strncpy(data1, buf, 256);
    ...
    if (fork() == 0) { //POTOMNY
        while (1) {
            ...
            sscanf(data1, "%d %d %d %d", &magazyn_A, &magazyn_B, &magazyn_C,
            ...
            if (liczba_A <= magazyn_A && liczba_B <= magazyn_B && liczba_C <= magazyn_C) {
                ...
                printf("Kurier 1, Odebrał zamówienie nr %d: %d %d %d\n", id_zamowienia, liczba_A, liczba_B, liczba_C);
                printf("Magazyn 1, Nie ma wystarczającej ilości towaru.\n");
                podnies_sem(semid, 0); //semafor
                exit(0);
            }

            sprintf(buf, "%d %d %d %d", magazyn_A, magazyn_B, magazyn_C, gold); //zapisz ilości do bufora
            strncpy(data1, buf, 256); //zapisz ilości do pamięci współdzielonej
            podnies_sem(semid, 0); //semafor
        ...
        if (fork() == 0) { // POTOMNY
            ...
            if (fork() == 0) { // POTOMNY
                ...
                printf("Stan magazynu 1 : A:%d B:%d C:%d\n", magazyn_A, magazyn_B, magazyn_C);
